...
### 作業系統  資訊安全策略:DAC    MAC    RBAC ABAC

```
https://zh.wikipedia.org/wiki/資訊安全策略

1.資訊安全策略是一種基於資訊安全需要所建立的準則，並非資訊安全細則，而是方向性的策略。
2.一般而言資訊安全策略有最大特權策略以及最小特權策略兩種，
   根據此二種策略又細分為存取控制方法、資料流控制方法、推論控制方法、以及資料加密方法。

最小特權策略:意謂著任何資料如果沒有被設定存取權限，則是當然不允許存取的。
最大特權策略:意謂著任何資料如果沒有設定存取權限，則默認為可以被存取。
...
強制存取控制（Mandatory Access Control, MAC)，
      以【管理者授權】為基礎，
      所有的資訊元件都需要經過管理者授權，才能被使用者所存取。

自由存取控制（Discretionary Access Control, DAC)，
     以【創建資訊元件的元件持有者授權】為基礎，
     不需要經過管理者授權，
     持有者授權可以決定使用者對於資訊元件的存取權限。

【角色】存取控制（Role-base Access Control），
     存取權限與使用者角色相依，
     資訊元件的存取被授權給角色，使用者需要先取得角色身份，才能透過角色身份取得存取權限。
     
     例如，某公司有經理人與一般職員兩個角色，經理人可以存取A與B兩元件，但職員只可以存取B元件。
     某甲進入公司之後取得職員身份，依據角色存取控制方法，某甲只能存取B資訊元件，直到他獲得經理人角色之後，
     才可以獲得A資訊元件的存取權限。
```
...
未經授權的回答:
名稱:    www.google.com
Addresses:  2404:6800:4012:1::2004
          172.217.160.100
```
```
...
### 攻擊作業系統  Rootkit
```
1.Rootkit是指其主要功能為：隱藏其他程式行程的軟體，可能是一個或一個以上的軟體組合；
2.廣義而言，Rootkit也可視為一項技術。
3.在今天，Rootkit一詞更多地是指被作為驅動程式，載入到【作業系統核心kernal 】中的惡意軟體。
因為其程式碼執行在特權模式之下，從而能造成意料之外的危險。
...
A1:2017-Injection　注入攻擊   command injection  sql injection
...
2.在設計與資料庫連結並存取資料時，在需要填入數值或資料的地方，使用參數（parameter）來給值，
3.這個方法目前已被視為最有效可預防SQL注入攻擊的攻擊手法的防禦方式。
```
```
...
```
A
Cookie（複數形態Cookies），又稱為「小甜餅」。類型為「小型文字檔案」，
指某些網站為了辨別【用戶】身分而儲存在【用戶端Client Side】上的資料（通常經過【加密】）。

由網景公司的前雇員盧·蒙特利在1993年3月發明。
最初定義於RFC 2109。
...
請求意見稿（英語：Request for Comments，縮寫：RFC），又翻譯作意見徵求，意見請求，
RFC是由網際網路工程任務組（IETF）發布的一系列備忘錄。
檔案收集了有關網際網路相關資訊，以及UNIX和網際網路社群的軟體檔案，以編號排定。
目前RFC檔案是由網際網路協會（ISOC）贊助發行。
...
SQL Injection  ==> 伺服器端的攻擊
XSS(Cross-Site Scripting) ==> client端的攻擊
Google-hacking ==> see

   Google Hacking:滲透性測試者的利劍(原書第3版)
   簡體中文/Johnny Long, Bill Gardner, Justin Bro/清華大學出版社出版日期：2017-11-21
...
BEEF Framework ==> The Browser Exploitation Framework   打瀏覽器
               https://www.youtube.com/watch?v=BzepbzdIoGc
               https://www.youtube.com/watch?v=eVfk04is_0E
...
```
1.跨站請求偽造（英語：Cross-site request forgery），
也被稱為 one-click attack 或者 session riding，通常縮寫為 CSRF 或者 XSRF， 
2.是一種挾制用戶在當前已登錄的Web應用程式上執行非本意的操作的攻擊方法。
3.跟跨網站指令碼（XSS）相比，
XSS 利用的是用戶對指定網站的信任，
CSRF 利用的是網站對用戶網頁瀏覽器的信任。
```
```
https://docs.microsoft.com/zh-tw/aspnet/core/security/anti-request-forgery?view=aspnetcore-5.0
```
```
...
```
```
Ensiko：具備勒索病毒能力的網站指令介面工具 (Webshell)   2020 年 08 月 05 日
https://blog.trendmicro.com.tw/?p=65396
```
```
美澳聯手警告：小心Web Shell惡意程式
有鑑於駭客利用網頁殼層（Web Shell）滲透企業網路，以自遠端執行任意系統命令的攻擊事件頻傳，
美國與澳洲政府發布《網路安全資訊指南》，以協助各組織偵測及預防Web Shell惡意程式
文/陳曉莉 | 2020-04-24發表
